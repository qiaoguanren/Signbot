...
from legged_gym.envs.h1.h1_mimic_hand import *
...
class H1MimicViewMotion(H1MimicEval):
                ...
                if len(save_ids) > 0:
                    for i in save_ids:
                        if not self.saved_flags[i]:
                            ...
                            # np.save(f'/data1/guanren/American_poses/target_body/{self.motion_names[i]}_key_bodies.npy', self.to_save_list[i].cpu().numpy())
                            np.save(os.path.join(ASE_DIR, f"ase/poselib/data/retarget_npy/{self.motion_names[i]}_w1_dof_pos.npy"), np.array(self.w1_dof_pos))
    ...
    def _motion_sync(self):
        ...
        root_pos, root_rot, dof_pos, root_vel, root_ang_vel, dof_vel, key_pos, hand_dof_pos, extra_wrist, extra_elbow \
           = self._motion_lib.get_motion_state(motion_ids, motion_times)
        ...
        self.gym.set_dof_state_tensor_indexed(self.sim,
                                              ...
                                              gymtorch.unwrap_tensor(env_ids_int32), len(env_ids_int32))
        if self.save:
            local_end_pos = global_to_local(self.base_quat, self.rigid_body_states[:, self._new_key_body_ids_sim, :3], self.root_states[:, :3])
            for i in range(self.num_envs):
                self.to_save_list[i][self.motion_frame[i], :, :] = local_end_pos[i, :]
        
        self.w1_dof_pos.append(torch.cat([dof_pos, hand_dof_pos, extra_wrist, extra_elbow], dim=-1).cpu().numpy())
        return
