...
class H1Mimic(LeggedRobot):
    ...
    def compute_observations(self):
        ...
        # if (self.episode_length_buf[1] * self.dt) >= self._motion_lengths[1]:
        #     # 保存数据到文件，只保存第二个环境的数据
        #     np.save('second_env_motion_data.npy', np.array(self.obs_demo_save))
        #     print("Motion data for second environment saved to second_env_motion_data.npy")
            
        #     # 清空保存的demo数据，准备下一轮的记录
        #     self.obs_demo_save = []
    ...
    def _reward_tracking_demo_dof_pos(self):
        ...
        # print('dof num', dof_pos.shape)
        # rew1 = torch.exp(-0.7 * torch.norm((self.dof_pos[:, self._dof_ids_subset] - demo_dofs)[:, 9:], dim=1))
        # rew2 = torch.exp(-0.1 * torch.norm((self.dof_pos[:, self._dof_ids_subset] - self.default_dof_pos[:, self._dof_ids_subset])[:, :9], dim=1))
    ...
    def _reward_tracking_mul(self):
        ...
        rew = rew_key_body * rew_roll_pitch * rew_ang_vel * rew_dof_pos #* rew_hand_dof_pos # * rew_dof_vel
