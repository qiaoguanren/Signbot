...
#
# PPO-Penalty version: adapted from your PPO-Clip code.
...
class PPOPenaltyMimic:
    def __init__(self,
                 ...
                 num_mini_batches=1,
                 clip_param=0.2,  # <-- Removed in PPO-Penalty
                 gamma=0.998,
                 lam=0.95,
                 value_loss_coef=1.0,
                 entropy_coef=0.0,
                 ...
                 # PPO-Penalty-specific
                #  beta=1.0,           ## PPO-Penalty CHANGE: KL penalty coefficient
                #  adaptive_beta=True, ## PPO-Penalty CHANGE: whether to adapt beta
                 **kwargs
                 ):
    ...
    def act(self, obs, critic_obs, info, hist_encoding=False):
        ...
        # Compute the actions and values, possibly using estimated priv_states
    ...
    def update(self):
        ...
        for sample in generator:
            ...
            priv_states_predicted = self.estimator(obs_batch[:, self.prop_start:self.prop_start + self.num_prop])
            ...
            ) if len(self.priv_reg_coef_schedual) > 3 else 0.0
            ...
            # KL divergence
            ## PPO-Penalty CHANGE: classical formula for diag Gaussians
            kl = torch.sum(
                torch.log(old_sigma_batch / sigma_batch + 1.e-8)
                + (torch.square(sigma_batch) + torch.square(mu_batch - old_mu_batch))
            ...
            # Optionally adapt beta so that KL ~ desired_kl
            ...
            loss = surrogate_loss \
                   ...
                   + self.beta * kl_mean  ## PPO-Penalty CHANGE
        ...
        mean_discriminator_loss = 0.0  # (Not used here)
        mean_discriminator_acc = 0.0   # (Not used here)
        
        self.storage.clear()
        self.update_counter()

        return (mean_value_loss, mean_surrogate_loss, mean_estimator_loss, 
                mean_discriminator_loss, mean_discriminator_acc, 
                mean_priv_reg_loss, priv_reg_coef)
